...
# Reference: Used reframe because given a warning when using summarise:
...
surveys |>
  summarize(across(everything(), ~ map_chr(.x, ~ class(.x)[1]))) |>
  slice_head()
...
evals <- evals |>
  bind_cols(
    map_at(
      evals,
      c(
        "teacher_id", "weekday", "academic_degree", "seniority", "gender"),
      as.factor
      )
  )
